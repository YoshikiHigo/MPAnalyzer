...
public class EXECUTEME {
	...
	public static Map<String, String> getConfigs() {
		final List<String> optionNames = CPAConfig.getOptions().stream()
				.map(o -> o.getLongOpt()).collect(Collectors.toList());
		final Map<String, String> configs = new HashMap<>();
		try {
			final List<String> lines = Files.lines(Paths.get(CONFIG)).collect(
					Collectors.toList());
			for (int i = 0; i < lines.size(); i++) {
				...
				if (optionNames.stream().noneMatch(n -> n.equals(configName))) {
					System.out.print("line ");
					System.out.print(Integer.toString(i + 1));
					System.out.print(": ");
