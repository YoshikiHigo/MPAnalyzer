...
public class PatternFilteringPanel extends JPanel implements Observer {
		...
		this.logKeywordField
				.addActionListener(e -> {
					...
					if (0 == tokenizer.countTokens()) {
						...
						final List<ChangePattern> allPatterns = ReadOnlyDAO.SINGLETON
								.getChangePatterns();
						final List<Integer> allPatternIDs = allPatterns
								.stream().map(p -> p.id)
	...
	private boolean includingKeyword(final ChangePattern pattern,
			final String keyword) {
		final List<Change> changes = ReadOnlyDAO.SINGLETON.getChanges(
				pattern.beforeHash, pattern.afterHash);
		return changes.stream().anyMatch(
				c -> c.revision.message.contains(keyword));
	}
}
