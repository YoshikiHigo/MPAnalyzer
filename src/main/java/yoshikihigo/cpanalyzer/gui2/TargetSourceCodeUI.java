...
class TargetSourceCodeUI extends BasicTextAreaUI {
  ...
  class SourceCodePlainView extends PlainView {
    ...
    @Override
    protected int drawUnselectedText(final Graphics g, final int x, final int y, final int p0,
    ...
    private void drawLangToken(final Graphics g, final int x, final int y, final String token) {

      g.setPaintMode();
      Color backupColor = g.getColor();
      g.setColor(new Color(12, 43, 116));
    ...
    protected void drawLine(final int lineIndex, final Graphics g, final int x, final int y) {
    ...
    private void drawLineNumber(final Graphics g, final int lineIndex, final int y) {
      ...
      int x_location = this.margin.left - 8 - g.getFontMetrics()
          .stringWidth(lineString);
