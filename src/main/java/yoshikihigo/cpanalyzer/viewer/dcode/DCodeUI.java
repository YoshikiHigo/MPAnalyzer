package yoshikihigo.cpanalyzer.viewer.dcode;
...
class DCodeUI extends BasicTextAreaUI {
  ...
  class SourceCodePlainView extends PlainView {
    ...
    public SourceCodePlainView(final Element elem, final JTextArea textArea, final Insets margin) {
      super(elem);
    ...
    protected int drawUnselectedText(Graphics g, int x, int y, int p0, int p1)
      ...
      int start = p0;
      final String buffer = this.textArea.getDocument()
          .getText(p0, p1 - p0 + 1);
    ...
    private void drawLineBackground(final Graphics g, final int lineIndex) {
      ...
      if (!DCodeUI.this.emphasizedLines.contains(lineIndex)) {
        return;
      }
      ...
      try {
        ...
        g.setColor(EMPHASIZED_LINE_COLOR);
        final int startOffset = this.textArea.getLineStartOffset(lineIndex);
        final Rectangle rectangle = DCodeUI.this.modelToView(this.textArea, startOffset);
        g.fillRect(0, rectangle.y, this.textArea.getWidth(), rectangle.height);
    ...
    private void drawLineNumber(final Graphics g, final int lineIndex, final int y) {

      final Color backupColor = g.getColor();

      g.setColor(VISUAL_SOURCECODEVIEW_LINENUM_COLOR);
      g.drawLine(this.margin.left - 5, y - g.getFontMetrics()
          .getHeight(), this.margin.left - 5, y);

      final String lineNumber = String.valueOf(lineIndex + 1);
      final int x_location = this.margin.left - 8 - g.getFontMetrics()
          .stringWidth(lineNumber);
  ...
  static public final Color EMPHASIZED_LINE_COLOR = new Color(0, 130, 130, 80);
  static public final Color AFTER_MODIFICATION_LINE_COLOR = new Color(130, 130, 0, 80);
  static public final Color VISUAL_SOURCECODEVIEW_LANG_TOKEN_COLOR = new Color(12, 43, 116);
  static public final Color VISUAL_SOURCECODEVIEW_LINENUM_COLOR = new Color(0, 100, 0);

  final private Set<Integer> emphasizedLines;
  final private JTextArea textArea;
  final Insets margin;

  DCodeUI(final Set<Integer> emphasizedLines, final JTextArea textArea, final Insets margin) {
