package yoshikihigo.cpanalyzer.viewer.ocode;
...
class OCodeUI extends BasicTextAreaUI {
  ...
  class SourceCodePlainView extends PlainView {
    ...
    private void drawLineBackground(final Graphics g, final int lineIndex) {
      ...
      if (!OCodeUI.this.emphasizedLines.contains(lineIndex)) {
        return;
      }
      ...
      try {
        ...
        final Rectangle rectangle = OCodeUI.this.modelToView(this.textArea, startOffset);
  ...
  OCodeUI(final Set<Integer> emphasizedLines, final JTextArea textArea, final Insets margin) {
