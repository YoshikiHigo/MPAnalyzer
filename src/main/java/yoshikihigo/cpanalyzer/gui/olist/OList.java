...
public class OList extends JTable implements Observer {
    ...
    public void valueChanged(final ListSelectionEvent e) {
      ...
      if (!codefragments.isEmpty()) {
        ObservedCodeFragments.getInstance(CFLABEL.OVERLOOKED)
            .setAll(codefragments, OList.this);
      } else {
        ObservedCodeFragments.getInstance(CFLABEL.OVERLOOKED)
            .clear(OList.this);
      }
  ...
  class OListPopupMenu extends JPopupMenu {
    ...
    OListPopupMenu() {
      final JMenuItem seeItem = new JMenuItem("see the details of the selected CP");
      this.add(seeItem);
      ...
      seeItem.addActionListener(new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          for (final int index : OList.this.getSelectedRows()) {
            final int modelIndex = OList.this.convertRowIndexToModel(index);
            final ChangePattern mp = (ChangePattern) model.oCodefragments.get(modelIndex)[0];
            mps.add(mp);
          }
  ...
  public void setModel(
    ...
    this.setModel(model);
    final RowSorter<OListModel> sorter = new TableRowSorter<OListModel>(model);
    ...
    this.getColumnModel()
        .getColumn(0)
        .setPreferredWidth(COLUMN_LENGTH_ID);
    this.getColumnModel()
        .getColumn(1)
  ...
  private List<Object[]> convertToList(
    ...
    for (final Entry<ChangePattern, SortedMap<String, SortedSet<Code>>> mEntry : oCodefragments
      ...
      final SortedMap<String, SortedSet<Code>> mCodefragments = mEntry.getValue();
      for (final Entry<String, SortedSet<Code>> pEntry : mCodefragments.entrySet()) {
        final String path = pEntry.getKey();
        final SortedSet<Code> pCodefragments = pEntry.getValue();
